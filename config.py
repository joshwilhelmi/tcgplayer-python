...
class ConfigurationManager:
    ...
    def load_from_env(self) -> ClientConfig:
        ...
        for env_var, config_key in env_mapping.items():
            ...
            if value is not None:
                ...
                if config_key in ["max_requests_per_second", "max_retries", "max_connections", 
                                "max_connections_per_host", "keepalive_timeout", "cache_ttl", 
                                "cache_max_size"]:
                    env_config[config_key] = int(value)
                elif config_key in ["rate_limit_window", "base_delay", "timeout_total", 
                                  "timeout_connect", "timeout_read"]:
                    env_config[config_key] = float(value)
                elif config_key in ["log_json_format", "enable_caching", "debug_mode", "mock_responses"]:
                    env_config[config_key] = value.lower() in ("true", "1", "yes", "on")
                elif config_key in ["log_level", "base_url", "api_version", "client_id", "client_secret", "log_file"]:
    ...
    def load_from_file(self, config_file: Optional[Union[str, Path]] = None) -> ClientConfig:
    ...
    def create_default_config(self, config_file: Optional[Union[str, Path]] = None) -> None:
...
def load_config(config_file: Optional[Union[str, Path]] = None,
                env_override: bool = True) -> ClientConfig:
    """
