...
class ParameterValidator:
    ...
    def validate_float_range(
        ...
        if min_value is not None and float_value < min_value:
            raise ValidationError(f"{param_name} must be >= {min_value}, got {float_value}")
        
        if max_value is not None and float_value > max_value:
            raise ValidationError(f"{param_name} must be <= {max_value}, got {float_value}")
    ...
    def validate_price(price: Union[int, float], param_name: str = "price") -> float:
        ...
        return ParameterValidator.validate_float_range(
            price, param_name, min_value=0.0
        )
    
    @staticmethod
    def validate_quantity(quantity: Union[int, float], param_name: str = "quantity") -> float:
        """
